...
###################### decode ######################
####################################################
function testBase64DecodeFunction() {
	printf "%b" "\n\n"
	printf "%b" "${BLUE}Test ${algo} with fd 0"
	printf "%b" "\n\n"

	algo="base64"
	for line in $(cat str_to_test_to_decode_base)
	do
		mine=$(echo -n ${line} | ${bin} ${algo} -d)
		if [[ ${IS_LINUX} == "true" ]]; then
			real=$(echo -n ${line} | base64 -d)
		else
			real=$(echo -n ${line} | base64 -d)
		fi
		if [[ -z $(diff <(echo ${mine}) <(echo ${real})) ]]; then
	...
	for line in $(cat file_c_decode_base64)
	do
		mine=$(echo -n ${line} | ${bin} ${algo} -d)
		if [[ ${IS_LINUX} == "true" ]]; then
			real=$(echo -n ${line} | base64 -d)
